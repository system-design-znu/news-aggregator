...
port = '5432'
database = 'RSS'
excelpath = 'D:\Programming\Python\PythonCode\SQL\RSS.xlsx'
txtpath = "D:\Programming\Python\PythonCode\SQL\Configfile.txt"
tableName = "RSS_Fetch"
...
user, password, host, port, database, excelpath, tableName = read_from_file(txtpath)
...
def write_from_excel(input_engine, excel_path, table_name):
    with pd.ExcelFile(excel_path) as xls:
        df = pd.read_excel(xls)
        df.to_sql(name=table_name, con=input_engine, if_exists='replace', index=False)


write_from_excel(engine, excelpath, tableName)
